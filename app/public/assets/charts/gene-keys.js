/**
 * gene-keys.js — the 64 Gene Keys spectrum, as reference data.
 *
 * WHY THIS EXISTS. The chart engine (hd-engine.js) computes which Gene Keys a
 * person carries — Life's Work, Evolution, Radiance, Purpose — but it returns
 * only GATE NUMBERS. Until this file existed, the companion was handed
 * "Life's Work 43" and nothing else, while its own voice instructions told it
 * to speak in Shadow / Gift / Siddhi terms. A model asked to name the Shadow of
 * gate 43 with no table in front of it will invent one, confidently. Guidance
 * built on an invented Shadow is worse than no guidance.
 *
 * So: the spectrum for all 64 keys, verified against genekeys.com per-key pages
 * and cross-checked against independent reference tables (2026-08-07).
 *
 * SCOPE — deliberately narrow. Only the three single-word spectrum labels per
 * key, which are reference data in the same way element names are. Richard
 * Rudd's long-form contemplative prose for each key is his copyrighted book
 * text and is NOT reproduced here, and must not be pasted in later.
 *
 * Gate 55 is not a typo: its Gift and its Siddhi are both Freedom.
 *
 * Usable three ways — ES module import, <script type="module">, and
 * window.FRQNCYGeneKeys for the non-module surfaces.
 */
...
/** The four spheres the intake computes, in the order they are contemplated. */
export const SPHERES = [
  { key: 'lifesWork', label: "Life's Work", of: 'how you work in the world' },
  { key: 'evolution', label: 'Evolution',   of: 'the relational challenge that grows you' },
  { key: 'radiance',  label: 'Radiance',    of: 'how your vitality shows' },
  { key: 'purpose',   label: 'Purpose',     of: 'what it is all in service of' },
];

/** One key, or null for anything that is not a real gate. */
export function geneKey(gate) {
  const n = Number(gate);
  if (!Number.isInteger(n) || n < 1 || n > 64) return null;
  return GENE_KEYS[n] || null;
}

/** "43 — Shadow Deafness → Gift Insight → Siddhi Epiphany" */
export function spectrumLine(gate) {
  const k = geneKey(gate);
  if (!k) return null;
  return `${Number(gate)} — Shadow ${k.shadow} → Gift ${k.gift} → Siddhi ${k.siddhi}`;
}

/**
 * Resolve a stored gk profile ({lifesWork, evolution, radiance, purpose} as
 * gate numbers) into the same shape with the spectrum attached. Unknown or
 * stub values drop out rather than resolving to a wrong key.
 */
export function resolveGeneKeys(gk) {
  if (!gk || typeof gk !== 'object' || gk._stub) return null;
  const out = {};
  let any = false;
  for (const s of SPHERES) {
    ...
    const k = geneKey(gate);
    if (!k) continue;
    out[s.key] = { gate: Number(gate), label: s.label, of: s.of, ...k };
    any = true;
  }
  return any ? out : null;
}
...
if (typeof window !== 'undefined') {
  window.FRQNCYGeneKeys = { GENE_KEYS, SPHERES, geneKey, spectrumLine, resolveGeneKeys };
}
