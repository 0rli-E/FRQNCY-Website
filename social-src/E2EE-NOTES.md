...
- **Group-creation UI:** `StartGroupConversation.tsx` renders as a modal triggered by the "+ Start group chat" button at the top of `ConversationsList` (visible only when signed in). Username search picks up to 9 other members (10 total including self), inserts a `conversations` row + N `conversation_members` rows, and navigates to `/social/messages/?cid=<new_conversation_id>`. No changes to `useMessages` — once membership has 2+ non-self members the existing v2 per-recipient encryption path takes over automatically. All members are equal — no admin/owner surface in v1.
