...
1. ~~**The Trail**~~ — **SHIPPED 2026-08-02** (`39be61d83`, on `vbrtn-live`, not yet pushed). Slide-out journal of past daily intentions + reflections paired with that day's habit completion. Read-only. Sixty days at a time, older on request. Empty days quietly skipped. Opened by a "Look back" link at the foot of the Today panel's evening review, which appears only once there is something behind you. Two judgement calls beyond the spec: a habit only counts against days it already existed on (so adding a habit can't retroactively make the past look incomplete), and today carries no ratio because the day isn't over. `renderTrail`/`trailEntries`/`openTrail` in the dashboard; no new state.
2. ~~**Weekly review**~~ — **SHIPPED 2026-08-02** (vbrtn-live, unpushed). Departures: the door shows daily but only becomes the gold invitation on Sunday when unwritten; the panel always reviews a *complete* ISO week so a midweek visitor never meets a part-week of zeros; last week's answers read back at the foot. Original spec — every Sunday, the Today panel offers a soft prompt: *"It's the week's edge. Look back?"* Click opens a synthesis: which goals were hit, habit ratios, chief-aim score deltas, and three contemplative questions (*What worked? What's calling for attention? What would last week's you have wanted to know?*). Reflections persist as `weeklyReviews[isoWeek]`. Same JSON path as everything else.
3. ~~**Monthly close**~~ — **SHIPPED 2026-08-02** (vbrtn-live, unpushed). Departures: "last day of the month" became a window through the 7th; past closes stay readable via a door in the Trail's foot; goals render as the pooled mirror, not a bare fraction; the word-count stoplist is a visible editorial object. Original spec — on the last day of each month, a single-pane summary: goals hit / total, dominant habits, the user's three most-written-about words across intentions and reflections. Optional one-line *"What did this month serve?"* — saved as the month's epigraph.
4. ~~**Constellation visit summary on the dashboard**~~ — **SHIPPED 2026-08-02** (vbrtn-live, unpushed). **The spec was not buildable as written:** the tracker's URL matcher still expected the `/v2/` prefix retired in May, so it had recorded nothing for ~3 months, and it stored a flat slug list with no counts or dates. Matcher fixed and a bounded month-granularity counter added (`frqncy:visited.v2`); history starts empty. Kept out of `DEFAULT_STATE` so reading history never syncs. Original spec — *"You've opened 14 topics this month. Three of them three or more times: Meditation, Sound Healing, Prosperity Mindset."* Reads from the existing visited-topics tracker. Honors the no-recommendation principle: this is observational, not algorithmic.
...
6. **One-click dreamboard add** — the current Vision Board upload is multi-step (open the tab, click upload, file picker, optional metadata, save). Reduce to a single tap from the Today panel and from any topic page: paste-from-clipboard, drag-and-drop onto a small Dreamboard-target affordance, screenshot-to-board, and a PWA share-target on mobile (so iOS/Android's native share sheet lists "Save to FRQNCY Dreamboard" alongside Photos/Notes). *Done* = the image lands on the board with no further form to fill. Optional chief-aim link is inferred from the topic context the image came from (e.g., adding an image while on `/permaculture/` pre-suggests the chief aim that touches that domain — user confirms or skips with one tap). The upload-into-private-IndexedDB path stays unchanged; only the input affordance changes. The aim is to make growing the dreamboard frictionless enough that it becomes the default response to "I want to remember this" — turning the board into the long-term visual companion to the daily ritual, not a feature you have to remember to use.
7. **Mindmovies — playable dreamboards** — the dreamboard becomes a short auto-playing sequence: the user's chosen images cross-fading on a slow ken-burns pan, optional affirmation lines drawn from their chief aims surfacing over each image (display font, low-key), and a soft audio bed (solfeggio tones, soundheal samples, or user-supplied music via the same one-click upload as images). A *Play your dream* button on the Today panel; ~2–4 minutes per loop; configurable cadence (morning, evening, or both — quiet nudge, not a notification storm). Rendering is client-side via canvas/WebAudio so the images never leave the device. Optional MP4 export via WebCodecs for users who want to take it onto their phone lock-screen or watch it offline. Per the *no-spiritual-materialism* principle: this is not "manifest harder" — it is the visual + auditory companion to a contemplative practice already in motion, the way a meditation cushion supports meditation. Mindmovies render the dream the user wrote into something they can sit with for three minutes a day without typing a word.
