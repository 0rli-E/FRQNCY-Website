...
function buildPage(c) {
  ...
  // HowTo schema — each lesson is a HowToStep. Pairs with the per-lesson
  // VideoObject already in the Course schema. Eligible for the HowTo rich
  // result + step-by-step previews in Google SERPs.
  const howToSteps = c.lessons.map((l, i) => {
    const step = {
      '@type': 'HowToStep',
      'position': i + 1,
      'name': l.title,
      'url': `${courseUrl}#lesson-${l.id || i + 1}`,
    };
    if (l.desc) step.text = l.desc;
    const vid = lessonVideoId(l);
    if (l.type === 'video' && vid) {
      step.image = `https://img.youtube.com/vi/${vid}/maxresdefault.jpg`;
    }
    return step;
  });
  const howToSchema = {
    ...
    'description': c.desc,
    'url': courseUrl,
    'totalTime': durationToISO(c.duration),
    'step': howToSteps,
  };
...
/* ── Build ── */
// Output paths: /<courses>/<slug>/index.html at repo root.
// Was v2/courses/ pre-2026-05-13; the /v2/ removal moved pages but the
// generator output dir was left pointing at the old location for weeks,
// so regens silently wrote to a stale directory while production served
// from the new path. Corrected here.
for (const c of courses) {
  const outDir = path.join(ROOT, 'courses', c.slug);
...
fs.writeFileSync(path.join(ROOT, 'courses', 'index.html'), buildHubPage(courses), 'utf8');
